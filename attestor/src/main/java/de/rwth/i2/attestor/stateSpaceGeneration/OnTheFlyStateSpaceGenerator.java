 ...
 *
 */
public class OnTheFlyStateSpaceGenerator {
    ...
    /**
     * Contains heap out of scope. Important for AP labelling of states of method calls.
     */
     ...
     */
    AutomatonStateLabelingStrategy stateLabelingStrategy;
    /**
     ...
     */
    public static OnTheFlyStateSpaceGeneratorBuilder builder() {
    ...
    public ScopedHeap getScopedHeap() {
    ...
    private void generateState(ProgramState state, Set<Node> formulae) throws StateSpaceGenerationAbortedException {
    	// , ScopedHeap globalScopedHeap to params
...
//            Collection<ProgramState> successorStates = stateSemanticsCommand.computeSuccessorsOnTheFly(state, formulae, globalScopedHeap);	            
            if(finalStateStrategy.isFinalState(state, successorStates, stateSemanticsCommand)) {
                stateSpace.setFinal(state);
                stateSpace.addArtificialInfPathsTransition(state); // Add self-loop to each final state
            } else {
    ...
    private void labelWithAtomicPropositions(ProgramState state) {
        ...
        if(state.isFromTopLevelStateSpace()) {
        	...
        	stateLabelingStrategy.computeAtomicPropositionsFromGlobalHeap(state, scopedHeap);
        }
    }
