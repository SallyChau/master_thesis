...
\begin{document}
	...
	%TODO describe algos as basis for own algos and how we changed them
	
	We assume that the transition system under consideration is finite and has no terminal states.
	
	\subsection{Automata-Based LTL Model Checking} 
	% TODO reference paper "Analysis of Recursive States Machines"
	% TODO runtime/ complexity of LTL model checking
	This section presents the basic ideas of the automata-theoretic approach to model checking transition systems for LTL specifications as introduced in \cite{vardi1986automata} by Vardi and Wolper. For an LTL formula $\varphi$ and a transition system $T$ the algorithm either returns "yes" if $T \models \varphi$ or "no" and a failure trace of the path that violates the formula $\varphi$. \\
	...
	The automata-based approach can be transferred to recursive state machines as described in \cite{alur2001analysis}. In this context, the notion of \textit{recursive Büchi automata} is introduced that augment RSMs with Büchi-acceptance conditions.\\	
	...
	This section presents an alternative approach to model checking a transition system $T$ for an LTL formula $\varphi$. The on-the-fly approach by Grumberg et al. constructs a \textit{proof structure} based on a set of \textit{tableaux rules} in order to proof if a formula is satisfied. \\
	
	A proof structure is a directed graph that depicts how the LTL formula $\varphi$ develops during the course of paths in the transition system $T$. The vertices of the proof structure represent \textit{assertions} of the form $s \vdash \Phi$, where $s$ is a state in $T$ and $\Phi$ is a set of LTL formulae. An assertion $s \vdash \Phi$ holds if at least one formula $\varphi \in \Phi$ is satisfied by the state $s$. The set of all assertions is denoted by $\Lambda$ while a single assertion is denoted by $\lambda$. The edges in the proof structure connect vertices that result from rule applications according to the tableaux, e.g., for two assertions $\lambda_1$ and $\lambda_2$, an edge connects the vertices if the tableaux defines a rule $\frac{\lambda_1}{\lambda_2}$.	
	...
	The tableaux rules the proof structure works with are specified in the following. They model the semantics of LTL.
	...
	\begin{theorem}[Correctness of the Tableaux Construction \cite{bhat1995efficient}]{thm:tableaux_correct}
		Given a concrete transition system $T$ with $s\in S$ and an LTL formula $\varphi$. Let $(V,E)$ be the proof structure for $s \vdash \{\varphi\}$. Then it holds that $s \models \varphi$ iff $(V,E)$ is successful.
	\end{theorem}
