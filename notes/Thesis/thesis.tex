...
\begin{document}
	...
	\begin{definition}[Hypergraph \cite{heinen2015verifyingPhd}]\label{def:hypergraph}
		Given a finite ranked alphabet $\Sigma$ with associated ranking function $rk : \Sigma \rightarrow \mathds{N}$. A (labelled) hypergraph over $\Sigma$ is a tuple \[H = (V ,E, att, lab, ext)\]
		where 
		\begin{itemize}
			...
			\item the hyperedge-labelling function $lab : E \rightarrow \Sigma$ maps to each edge its label, and 
	...
	An example for a hypergraph is depicted in Figure\ref{fig:dll_hyper}.
		...
		\begin{center}
			...
			\end{tikzpicture}}
			\caption{A doubly-linked list represented as a hypergraph. Adopted from \cite{heinen2015verifying}.}\label{fig:dll_hyper}
		\end{center}
	\end{figure}
