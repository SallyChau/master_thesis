...
\begin{document}
	...
	Given a transition system $T$ and a set of LTL formulae $\Phi$, the goal of the hierarchical tableaux construction is to verify whether the underlying state space including procedure state spaces induced by method executions satisfies the temporal properties specified by $\Phi$. We assume the state space to be flat and finite. The basis of the algorithm is the tableaux construction by Grumberg et al. that we extend by the following functionalities:
	\begin{itemize}
...
%TODO add more details
	...
	The algorithm is based on the procedure \texttt{generateAndCheck} that model checks a set of formulae $\Phi$ for an input program based on the algorithm of the \textsc{Attestor} state space generation described in Section~\ref{sec:attestor}. After initialising the state space and the proof structure, the proof structure for the initial states is constructed based on the tableaux rules defined in Section~\ref{sec:tableaux}. Successor states are generated on demand if the proof structure needs to be extended to further states due to validating $\bigcirc$-formulae. Statements that invoke method executions trigger the model checking of procedure state spaces, that is the procedure \texttt{generateAndCheck} is executed on the calling state of the method and the currently checked set of formulae $\Phi'$. It follows that a hierarchy of calls to \texttt{generateAndCheck} is constructed where each level represents the model checking procedure of a method execution. Once the procedure is completed for a procedure state space, the successor states and model checking results are returned to the above lying level. Model checking results include information on whether any formula is violated as well as the updated set of formulae for which the successor states are checked for. The model checking results are stored in contracts as pairs of input-and-output formulae such that they can be used to avoid repetitive model checking a procedure state space for the same set of input formulae.
	
	%TODO example for model checking contract
	
	Furthermore, if the proof structure for any procedure state space evaluates to \texttt{false}, the proof structure of the above lying state spaces can be aborted as a violating path has been found. Thus, an early termination of the model checking procedure is possible that does not require building the complete state space. Together with the updated set of formulae the resulting successor states induce new assertions that are added to the proof structure. If the model checking procedure has been successful, then the new assertions are added to the proof structure and the procedure is continued, otherwise, a violating path has been found and the proof structure is declared to be unsuccessul such that it can be concluded that the set of formulae $\Phi$ is not satisfied. The concept of the hierarchical tableaux construction is depicted in Figure~\ref{fig:otfHMC}.\\ 
	...
	refer to advantages of the three aspects:
