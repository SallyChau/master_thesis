...
\begin{document}
	...
	\begin{figure}
		\begin{center}
			%\resizebox{0.5\textwidth}{!}{
				...
				\node (r1) at (0, 2) [rectstyle] {\texttt{head}};
				...
				\path[] (r1) edge[line width=0.75mm] node[fill=white, right] {1} (a1);
	...
	\begin{figure}
		\begin{center}
			%\resizebox{0.5\textwidth}{!}{
				...
				\node (a4) at (4.5,0) [normal] {};
				\node (a5) at (6,0) [normal,fill=gray] {};
				\node (r1) at (1.5, 2) [rectstyle] {\texttt{head}};
				...
				\path[] (r1) edge[line width=0.75mm] node[fill=white, right] {1} (a2);
				
				\end{tikzpicture}%}
			\caption{Modified heap of a singly-linked list after pointer-operation \texttt{head := head.next}.}\label{fig:sll_op}
		\end{center}
	\end{figure}		
	...
	For a path $\pi$, $\pi[i]$ denotes the $i$th state of $\pi$, while  $\pi[i..]$ denotes the $i$th suffix of $\pi$. Paths display the order of states that are traversed throughout a sequence of transitions. However, the related sets of atomic propositions of the traversed states, which are relevant for model checking, are not observable in the path itself. Therefore, we consider the notion of \textit{traces} which are sequences of sets of atomic propositions that are satisfied along a path $\pi$. We regard transition systems without terminal states. This condition is not a restriction, since for every transition system an equivalent one without terminal states can be constructed. This is achieved by adding a state $s_{stop}$ with a self-loop to the transition system to which all terminal states have an outgoing edge.
	
	\begin{definition}[Trace \cite{baier2008principles}]{def:trace}
		Let $T=(S, I, \delta, AP, L)$ be a transition system without terminal states. The \textit{trace} of a finite path $\pi=s_0 s_1 \dots s_n$ is defined as \[\mathsf{trace}(\pi)=L(s_0)L(s_1)\dots L(s_n).\] The trace of an infinite path $\pi=s_0 s_1 \dots$ is defined as \[\mathsf{trace}(\pi)=L(s_0)L(s_1)\dots.\]
		$\mathsf{Traces}(s)$ denotes the set of traces of paths starting in state $s$ and $\mathsf{Traces}(T)$ denotes the set of traces of the initial states of a transition system $T$.
	\end{definition}
	...
	\begin{definition}[Recursive State Machine \cite{alur2001analysis}]{def:rsm}
		A \textit{recursive state machine} (RSM) $\mathcal{A}$ over a finite alphabet $\Sigma$ is given by a tuple $(A_1, ..., A_k)$, where each \textit{component state machine} \[A_i = (S_i, En_i, Ex_i, \delta_i, L_i),\] $1 \leq i \leq k$, consists of
		\begin{itemize}
			\item a set $S_i=N_i \cup B_i$ of states composing of a set $N_i$ of \textit{nodes} and a (disjoint) set $B_i$ of \textit{boxes},
			\item a set of \textit{entry nodes} $En_i \subseteq N_i$,
			\item a set of \textit{exit nodes} $Ex_i \subseteq N_i$, 
			...
			\end{itemize}
			\item a \textit{labeling} $L_i: B_i \mapsto \{1, ..., k\}$ that assigns to every box an index $j \in \{1, ..., k\}$ referring to one of the component state machines $A_1, ..., A_k$.
		\end{itemize}
	\end{definition}
	...
	Based on the satisfaction relation of LTL formulae over paths and states, we specify the semantics of LTL for a transition system $T$.
	...
	This section presents the on-the-fly approach by Grumberg et al. \cite{bhat1995efficient} that constructs a \textit{proof structure} based on a set of \textit{tableaux rules} in order to show whether an LTL formula $\varphi$ in PNF is satisfied by a transition system $T$. We assume all LTL formulae to be in PNF which is not a restriction due to Theorem~\ref{thm:pnf}. \\
	
	A proof structure is a directed graph $(V,E)$, where the set of vertices $V$ are composed of \textit{assertions} and the set $E$ of edges contains the edge $(\lambda_1, \lambda_2)$ between two assertions $\lambda_1$ and $\lambda_2$ if the underlying tableaux contains the inference rule $\frac{\lambda_1}{\lambda_2}$.
	...
	\begin{theorem}[Correctness of the Tableaux Construction \cite{bhat1995efficient}]{thm:tableaux_correct}
		Given a concrete transition system $T=(S, I, \delta, AP, L)$ with $s\in S$ and an LTL formula $\varphi$. Let $(V,E)$ be the proof structure for $s \vdash \{\varphi\}$. Then it holds that $s \models \varphi$ if and only if $(V,E)$ is successful.
	\end{theorem}
