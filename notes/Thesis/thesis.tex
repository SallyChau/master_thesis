...
%	\end{lstlisting}
	...
	Pointer-manipulating operations are represented by graph transformations. For instance, executing the operation \texttt{tail := head.next} on the heap given in Figure~\ref{fig:sll} attaches the variable \texttt{tail} to the vertex pointed to by the \texttt{next} selector of the first vertex. The resulting heap is shown in Figure~\ref{fig:sll_op}.\\ 
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.75\textwidth}{!}{
				...
				\node (a5) at (8,0) [normal] {};
				\node (r1) at (-2, 0) [rectstyle] {\texttt{head}};
				\node (r2) at (2, 2) [rectstyle] {\texttt{tail}};
				...
				\path[] (r1) edge[line width=0.75mm] node[fill=white, below] {1} (a1);
				\path[] (r2) edge[line width=0.75mm] node[fill=white, right] {1} (a2);
				
				\end{tikzpicture}}
			\caption{Modified heap of a singly-linked list after pointer-operation \texttt{tail := head.next}.}\label{fig:sll_op}
		\end{center}
	\end{figure}	
	
	The number of objects in a heap can become unboundedly large. Consider a program that contains a loop in which a new element is added to a list with every iteration. If the loop is visited infinitely often, the resulting heap size will increase with every iteration. In order to obtain a finite representation of a heap, parts of the graph are abstracted, i.e., a subgraph is replaced by a placeholder. An example for an abstracted graph is depicted in Figure~\ref{fig:sll_hyper}. The graph shows a singly-linked list with a placeholder labeled \textit{SLL} which represents a singly-linked list of arbitrary length. The example illustrates the concept of \textit{hypergraphs}. Hypergraphs are graphs in which an edge is connected to an arbitrary number of nodes. These edges are called \textit{hyperedges}. The number of vertices a hyperedge connects is captured in its \textit{rank}. Hypergraphs represent heaps that are partially concrete and partially abstract. In our example from Figure~\ref{fig:sll_hyper}, the hyperedge labeled \textit{SLL} is an abstracted part of the hypergraph in the otherwise concrete graph. In order to express the abstracted parts of the heap, we require a \textit{ranked alphabet} $\Sigma = \Sigma_N \uplus \Sigma_T$, where $\Sigma_N$ denotes a finite set of \textit{nonterminal symbols} and $\Sigma_T=Var \uplus Sel$ denotes the terminal symbols including the set $Var$ of variables and the set $Sel$ of selectors. Program variables are of rank one, while selectors are of rank two. Hypergraphs over the alphabet $\Sigma_T$ describe \textit{concrete} heaps that do not contain an abstract part such as the heap depicted in Figure~\ref{fig:sll}.\\	
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.9\textwidth}{!}{
				...
				\node (r) at (6, 0) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.9\textwidth}{!}{
				...
				\node (SLL) at (8, 0) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.9\textwidth}{!}{
				...
				\node (r) at (7, 3) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
				...
				\node (SLL) at (7,6) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
				...
				\node (r) at (8, 0) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
				...
				\end{tikzpicture}}
			\caption{By applying the first production rule from Figure~\ref{fig:sll_grammar} a list element is added to the (abstract) hypergraph.}\label{fig:sll_hyper_r1}
		\end{center}
	\end{figure}
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.9\textwidth}{!}{
				...
				\node (r) at (6, 3) [draw, fill=gray!10, minimum height=1cm] {$SLL$};
	...
	\begin{definition}[Terminal State \cite{baier2008principles}]{def:terminal_state}
		...
		\[\bigcup\; \{s'\in S\;|\;(s, s') \in \delta\} = \emptyset.\]
	\end{definition}
	...
	\begin{figure}
		\begin{center}
			\resizebox{0.65\textwidth}{!}{
				...
				%state 1
				\node (back1) at (-0.5,0) [rectstyle] {};
				\node (text) at (-2.25,1.15) {$s_0^{\texttt{swap}}$};
				\node (1) at (-0.75,0) [normal] {};
				\node (2) at (0.25,0) [normal] {};
				\node (3) at (1.25,0) [normal,fill=gray] {};
				\node (head) at (-1.85,0) [var] {\texttt{head}};
				...
				%state 2
				\node (back2) at (6,0) [rectstyle] {};
				\node (text) at (4.25,1.15) {$s_1^{\texttt{swap}}$};
				\node (1) at (5.75,0) [normal] {};
				\node (2) at (6.75,0) [normal] {};
				\node (3) at (7.75,0) [normal,fill=gray] {};
				\node (head) at (4.65,0) [var] {\texttt{head}};
				...
				\node (text) at (4.25,-2.85) {$s_2^{\texttt{swap}}$};
				...
				\node (text) at (-2.25,-2.85) {$s_3^{\texttt{swap}}$};
	...
	\begin{definition}[Paths \cite{baier2008principles}]{def:paths}
		...
		\begin{itemize}
			...
			\item $s_i \in \bigcup\; \{s\in S\;|\; (s_{i-1}, s) \in \delta\}$ for all $0 < i\leq n$, where $n \geq 0$, and 
		...
		\begin{itemize}
			...
			\item $s_i \in \bigcup\; \{s\in S\;|\; (s_{i-1}, s) \in \delta\}$ for all $i > 0$.
		\end{itemize}
	...
	\begin{definition}[Trace \cite{baier2008principles}]{def:trace}
		Let $T=(S, I, \delta, AP, L)$ be a transition system without terminal states.  The \textit{trace} of the finite path $\pi=s_0 s_1 \dots s_n$ is defined as \[\mathsf{trace}(\pi)=L(s_0)L(s_1)\dots L(s_n).\] The \textit{trace} of the infinite path $\pi=s_0 s_1 \dots$ is defined as \[\mathsf{trace}(\pi)=L(s_0)L(s_1)\dots.\]
	...
	\begin{definition}[Recursive State Machine \cite{alur2001analysis}]{def:rsm}
		A \textit{recursive state machine} (RSM) $\mathcal{A}$ over a finite alphabet $\Sigma$ is given by a tuple $(A_1, ..., A_k)$, where each \textit{component state machine} (CSM) \[A_i = (S_i, En_i, Ex_i, \delta_i, L_i),\] $1 \leq i \leq k$, consists of
		\begin{itemize}
	...
	\begin{definition}[Labeled Transition System induced by an RSM \cite{alur2001analysis}]{def:rsm_transitionSystem}
		The \textit{labeled transition system} (LTS) $T_\mathcal{A}=(S, \Sigma, \delta)$ induced by an RSM $\mathcal{A}=(A_1, ..., A_k)$ consists of
		\begin{itemize}
			\item the set of global states $S$, 
	...
	\begin{figure}
		\begin{center}
			\resizebox{\textwidth}{!}{
				...
				\node[rectstyle] (1) at (-0.75,-3) {$s_0^{\texttt{swap}} \vdash \{\varphi\}$};
				\draw[->,shorten >=0.5pt] (0) -- node[right] {$R^{\bigcirc}$} (1);
				
				\node[rectstyle] (3) at (-5.5,-6) {$s_0^{\texttt{swap}} \vdash \{\{\texttt{identicNeighbours}\}\}$};
				\node[rectstyle] (4) at (4,-6) {$s_0^{\texttt{swap}} \vdash \{\neg\texttt{true}, \bigcirc\varphi\}$};
				...
				\node[rectstyle] (5) at (4,-9) {$s_0^{\texttt{swap}} \vdash \{\bigcirc\varphi\}$};
				\draw[->,shorten >=0.5pt] (4) -- node[right] {$R^{\nvDash}$} (5);
				
				\node[rectstyle] (6) at (4,-12) {$s_1^{\texttt{swap}} \vdash \{\varphi\}$};
				\draw[->,shorten >=0.5pt] (5) -- node[right] {$R^{\bigcirc}$} (6);
				
				\node[rectstyle] (7) at (0,-15) {$s_1^{\texttt{swap}} \vdash \{\{\texttt{identicNeighbours}\}\}$};
				\node[rectstyle] (8) at (8,-15) {$s_1^{\texttt{swap}} \vdash \{\neg\texttt{true}, \bigcirc\varphi\}$};
				...
				\node[rectstyle] (9) at (8,-18) {$s_1^{\texttt{swap}} \vdash \{\bigcirc\varphi\}$};
				\draw[->,shorten >=0.5pt] (8) -- node[right] {$R^{\nvDash}$} (9);
				
				\node[rectstyle] (10) at (8,-21) {$s_2^{\texttt{swap}} \vdash \{\varphi\}$};
				\draw[->,shorten >=0.5pt] (9) -- node[right] {$R^{\bigcirc}$} (10);
				
				\node[rectstyle] (11) at (4,-24) {$s_2^{\texttt{swap}} \vdash \{\{\texttt{identicNeighbours}\}\}$};
				\node[rectstyle] (12) at (12,-24) {$s_2^{\texttt{swap}} \vdash \{\neg\texttt{true}, \bigcirc\varphi\}$};
				\draw[->,shorten >=0.5pt] (10) -- node[above] {$R^{\textbf{R}}_1$} (11);
				\draw[->,shorten >=0.5pt] (10) -- node[above] {$R^{\textbf{R}}_2$} (12);
				
				\node[rectstyle,draw=red,line width=2pt] (12) at (4,-27) {$s_2^{\texttt{swap}} \vdash \emptyset$};
				\draw[->,shorten >=0.5pt] (11) -- node[left] {$R^{\nvDash}$} (12);
				
				\end{tikzpicture}}
			\caption{Proof structure for model checking method \texttt{SLList.swap} for formula $\varphi=\square\{\texttt{identicNeighbours}\}$.}\label{fig:tableaux_swap}
		\end{center}
	\end{figure}
